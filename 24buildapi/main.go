...
func main() {
	...
	courses = append(courses, Course{CourseId: "2",
		...
		Auther: &Auther{Fullname: "Pratik R", Website: "pmr.dev"}})

	courses = append(courses, Course{CourseId: "4",
		CourseName: "MERN Stack", CoursePrice: 199,
		Auther: &Auther{Fullname: "Shoyeb T", Website: "shoyeb.dev"}})
...
func deleteOneCourse(w http.ResponseWriter, r *http.Request) {
	...
	for index, course := range courses {
		if course.CourseId == params["id"] {
			courses = append(courses[:index], courses[index+1:]...)
			//TODO: send a confirm or denied response
			break
		}
	}
